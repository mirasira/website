...
let cub = new cube([width/2, height/2, 0], 100);
...
function init(){
  ...
  for(let i = 0; i < NumberOfElemnts; i++){
    array.push(new rectangl(Math.random()*horizontSize-horizontSize/2, Math.random()*horizontSize-horizontSize/2, Math.random()*furthest, recsize, recsize, color[Math.floor(Math.random()*color.length)]));//Math.random()*300-150
  }
